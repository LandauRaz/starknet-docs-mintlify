...
* Introducing builtin tracking and temporarily disabling Cairo Native: The [Starknet OS](/architecture/os/) now tracks individual builtins usages, [capping the number of builtins that may fit inside each block](/resources/chain-info/#block_builtin_limits). Since [Cairo Native](https://github.com/lambdaclass/cairo_native) does currently not support this kind of tracking, its usage is temporarily disabled and will be enabled again once this capability is supported.

  <Note>
    Cairo Native hasn’t yet been enabled on Mainnet, and disabling it only affects Sepolia.
  </Note>

* Reducing maximum block size: The amount of L2 gas that a block can contain is [decreased from 5B to 4B L2 gas](/resources/chain-info/#current_limits).
