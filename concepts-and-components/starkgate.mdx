...
All procedures are subject to [fees](/concepts-and-components/messaging/#l1-l2-message-fees) to account for L1→L2 message costs. Fees for depositing funds or enrolling a token bridge on StarkGate can be estimated using the [`estimateDepositFeeWei`](/resources-and-references/starkgate-reference/#estimateDepositFeeWei) or [`estimateEnrollmentFeeWei`](/resources-and-references/starkgate-reference/#estimateEnrollmentFeeWei) function.

To invoke onchain contracts, use [Starknet Foundry's `sncast`](https://foundry-rs.github.io/starknet-foundry/starknet/sncast-overview.html), [Starkli](/ecosystem/tools/), or a [block explorer](/ecosystem/integrations/). To get the onchain addresses of StarkGate's contracts, see [Important addresses](/resources-and-references/chain-info/#starkgate).

For more information on StarkGate's various functions, see [StarkGate functions and events](/resources-and-references/starkgate-reference/).
...
3. The sequencer waits for enough L1 block confirmations to fill its quota to run before the corresponding deposit transaction is initiated on L2. During this period of time, the status of the L2 deposit transaction is [`NOT_RECEIVED`](/concepts-and-components/transactions/#transaction_lifecycle).
...
The status of the deposit request is now [`ACCEPTED_ON_L2`](/concepts-and-components/transactions/#transaction_lifecycle).
