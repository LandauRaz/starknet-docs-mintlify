...
SNOS is the program required for updating Starknet's state on Ethereum ([up to some caveats](#os-and-core-contract)). As such, is it also the final arbiter on what does it mean for a transaction to execute correctly. For example, while a malicious sequencer can deviate from [the `INVOKE` transaction's behavior](/concepts-and-components/transactions/#transaction_flow) by skipping the execution of `__validate__` before `__execute__` in the calling account contract, this specification is [enforced by SNOS](https://github.com/starkware-libs/cairo-lang/blob/8e11b8cc65ae1d0959328b1b4a40b92df8b58595/src/starkware/starknet/core/os/execution/execute_transactions.cairo#L390) and therefore this sequencer will not be able to produce the proof for the block in which this transaction was included.
...
* Verifying that all [L1→L2 messages](/concepts-and-components/messaging/#l1-l2-messages) were sent on Ethereum
