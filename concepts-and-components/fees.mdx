...
Every transaction on Starknet requires a small fee to process. The components contributing to this fee include [L2 computation, L2 data, and L1 data](/concepts-and-components/fees#fee-components), which are measured in [L2 gas, L1 gas, and L1 data gas](/concepts-and-components/fees#fee-resources). A transaction's fee can be [estimated](/concepts-and-components/fees#fee-estimations) and [limited](/concepts-and-components/fees#fee-limits), and is ultimately [charged](/concepts-and-components/fees#fee-charges) based on a predefined [formula](/concepts-and-components/fees#overall-fee).

<Note>
This section describes fees that are paid on L2 starting in Starknet 0.13.0. For information about messaging fees that are paid on L1, see [L1→L2 message fees](/concepts-and-components/messaging/#l1-l2-message-fees).
</Note>
...
* [L2 computation](/concepts-and-components/fees#l2-computation)

* [L2 data](/concepts-and-components/fees#l2-data) (calldata, events and code)

* [L1 data](/concepts-and-components/fees#l1-data), which includes:

  * The cost of posting the state diffs induced by the transaction to L1 (for more details, see [Data availability](/concepts-and-components/data-availability/))
...
Starting in Starknet v0.14.0, all transactions will have to specify STRK-denominated `max_amount` and `max_price_per_unit` for each of the three [fee unit](/concepts-and-components/fees#fee-units). A necessary condition for a transaction to enter a block is that `max_price_per_unit` is greater than or equal to the [block's corresponding price](/concepts-and-components/fees#gas-prices) for each resource, but if included sequencers are then enforced by the [Starknet OS](/concepts-and-components/snos/) (and as such, enforced by a proof) to charge no more than the max price specified by the transaction.
...
Sequencers usually charges less than the max price specified in the transaction, as charges are in accordance with the [formula for determining a transaction's fee](/concepts-and-components/fees#overall-fee).
</Note>
...
<Note>
While sequencer are [enforced not to overcharge fees](/concepts-and-components/fees#fee_limits), they are not yet enforced to charge fees charged according to Starknet's fee formula.
</Note>
...
- $ \text{l1}_{\text{gas price}} $ is the L1 gas price of the block that includes the transaction (see [Gas prices](/concepts-and-components/fees#gas-prices) for more information)
...
- $ \text{log}_{\text{message to l1 cost}} $ is 1,637 gas (see [L2→L1 messages](/concepts-and-components/fees#l2→l1-messages) for more information)

- $ \text{l2}_{\text{gas price}} $ is the L2 gas price of the block that includes the transaction (see [Gas prices](/concepts-and-components/fees#gas-prices) for more information)

- $ \text{tip} $ is the tip specified by the transaction

- $ \text{sierra}_{\text{gas consumed}} $ is the amount of [Sierra gas](/concepts-and-components/fees#sierra-gas) charged for computation of the transaction

* $$v$$ is a vector that represents resource usage of the transaction (Cairo steps or number of applications of each builtin), where each of its entries, $$v_{k}$$, corresponds to the usage of a different resource type (see [VM resources](/concepts-and-components/fees#vm-resources) for more information)

  <Note>
  The fee formula of a transaction can track both raw VM resources (reflected by $$v_{k}$$) and Sierra gas, depending on what classes it goes through (see [L2 computation](/concepts-and-components/fees#l2-computation) for more details).
  </Note>

* $$w$$ is the `CairoResourceFeeWeights` vector (see [VM resources](/concepts-and-components/fees#vm-resources) for more information)

- $ \text{l2}_{\text{payload costs}} $ is the gas cost of the data sent by the transaction over Starknet, which includes calldata, code, and event emission (see [L2 data](/concepts-and-components/fees#l2-data) for more information)

- $ \text{l1}_{\text{da mode}} $ is either $ \text{CALLDATA} $ or $ \text{BLOB} $, depending on how the state diff of the block that includes the transaction is sent to L1 (see [Data availability](/concepts-and-components/data-availability/) for more information)

- $ \text{l1}_{\text{data gas price}} $ is the L1 data gas price of the block that includes the transaction (see [Gas prices](/concepts-and-components/fees#gas-prices) for more information)
...
- $ \text{contract}_{\text{update discount}} $ is 312 gas (see [Storage updates](/concepts-and-components/fees#storage-updates) for more information)

- $ \text{sender}_{\text{balance update discount}} $ is $ 240 $ gas (see [Storage updates](/concepts-and-components/fees#storage-updates) for more information)
...
* $$\text{TARGET}$$ is 2 \* 109 ([half of Starknet's block capacity](/resources-and-references/chain-info/#current_limits))
...
* For Sierra ≥ 1.7.0, computation is measured in [Sierra gas](/concepts-and-components/fees#sierra-gas)

* For CairoZero classes or Sierra ≤ 1.6.0, computation is measured in [VM resources](/concepts-and-components/fees#vm-resources)
...
For information on the exact data and its construction, see [Data availability](/concepts-and-components/data-availability#v0-11-0).
</Note>
...
<Note>
This formula only refer to the case of submitting data to L1 via blobs, for the calldata case, see [Overall fee](/concepts-and-components/fees#overall-fee)).
</Note>
