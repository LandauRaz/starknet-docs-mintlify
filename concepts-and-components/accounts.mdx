...
Accounts represent users onchain, and enable them to interact with the blockchain. [Unlike Ethereum](/concepts-and-components/accounts#ethereum's-account-structure), Starknet facilitates account abstraction natively, via its unique account [structure](/concepts-and-components/accounts#starknet's-account-structure) and [nonce management](/concepts-and-components/accounts#account-nonces). Accounts can be [easily deployed on Starknet](/concepts-and-components/accounts#deploying-a-new-account), both with or without having a preexisting account.
...
<Tip>
For more information on transaction types and lifecycle, see [Transactions](/concepts-and-components/transactions/).

For more information on the each transaction's fields, see [Transaction fields and hash calculations reference](/resources-and-references/transactions-reference/).
</Tip>
...
<Tip>
To learn more about transaction statuses, see [Transactions](/concepts-and-components/transactions#transaction_statuses).
</Tip>
...
Thanks to account abstraction, the logic of `__execute__` and the different validation functions is up to the party implementing the account. To review a concrete implementation, you can check out OpenZeppelin's [account component](https://github.com/OpenZeppelin/cairo-contracts/blob/v0.14.0/src/account/account.cairo#L72), which also adheres to [SNIP6](/concepts-and-components/accounts#snip-6).
...
However, unlike Ethereum, only the nonce of account contracts — that is, those adhering to [Starknet's account structure](/concepts-and-components/accounts#starknet's-account-structure) — can be non-zero in Starknet. In contrast, in Ethereum, regular smart contracts can also increment their nonce by deploying smart contracts (i.e., executing the `CREATE` and `CREATE2` opcodes).
...
3. Charges fees from the new account address.
  ...
  <Note>
  If you use a `DEPLOY_ACCOUNT` transaction, the fees are paid from the address of the deployed account. If you use the UDC, which requires an `INVOKE` transaction, the fees are paid from the sender's account. For information on the differences between V1 and V3 `INVOKE` transactions, see [`INVOKE` transaction](/concepts-and-components/transactions#transaction_types) in *Transaction types*.
  </Note>
