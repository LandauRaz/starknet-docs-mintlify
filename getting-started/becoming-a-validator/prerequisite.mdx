...
Following this guide requires Docker, an exported Ethereum URL and three exported Starknet account. This installment of the guide will therefore walk you through [installing Docker](/getting-started/becoming-a-validator/prerequisite#installing-docker) and exporting [an Ethereum URL](/getting-started/becoming-a-validator/prerequisite#exporting-an-ethereum-url) and [the required accounts](/getting-started/becoming-a-validator/prerequisite#exporting-accounts).
...
<Note>
To understand the role of the each account, read more about [validator addresses in the staking protocol](/concepts-and-components/staking#addresses).
</Note>
...
* An `sncast` account named `staker` holding at least [the minimum required amount of STRK tokens](/concepts-and-components/staking#protocol) with its address exported using:
  ...
  <Tip>
  For deploying and funding an `sncast` account on Sepolia, see [Deploying a new Sepolia account](/getting-started/builders-quickstart/sepolia#deploying-a-new-sepolia-account).
  </Tip>