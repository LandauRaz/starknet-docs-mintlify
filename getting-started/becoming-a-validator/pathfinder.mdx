...
Starting from its second phase, the staking protocol also requires validators to prove they are actively preserving the history of the network by running a [Starknet full node](/ecosystem/integrations/) and submitting attestations to randomly assigned blocks in each epoch. This installment of the series will therefore walk you though [running a Pathfinder node](/getting-started/becoming-a-validator/pathfinder#running-a-pathfinder-node) and [its attestation service](/getting-started/becoming-a-validator/pathfinder#running-pathfinder's-attestation-service).
