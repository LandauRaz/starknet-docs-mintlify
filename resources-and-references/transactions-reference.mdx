...
* `h` is the [Poseidon](/concepts-and-components/cryptography/#poseidon_hash) hash.
...
For more information, see [Deploying a new account](/concepts-and-components/accounts/#deploying_a_new_account).
...
* `h` is the [Pedersen](/concepts-and-components/cryptography/#pedersen_hash) hash
...
* `h` is the [Poseidon hash function](/concepts-and-components/cryptography/#poseidon_hash)
* `class_hash` is the hash of the contract class. See [the Cairo Book](https://book.cairo-lang.org/ch100-01-contracts-classes-and-instances.html) for details about how the hash is computed
* `compiled_class_hash` is the hash of the compiled class generated by the Sierra→Casm compiler that is used in Starknet
...
* `h` is the [Pedersen](/concepts-and-components/cryptography/#pedersen_hash) hash and `sn_keccak` is [Starknet Keccak](/concepts-and-components/cryptography/#starknet_keccak).
* `chain_id` is a constant value that specifies the network to which this transaction is sent. For more information, see [Chain ID](#chain_id).
